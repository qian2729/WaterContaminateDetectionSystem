function [ C, sigma,TPR, FPR ] = ga_optimization(  train_data, train_label,validate_data,validate_label,alpha )
...
%     options.UseParallel='always';                     %Set parallel computation mode  并行运算（可以多个CPU运行）
    ...
    %-------------------------------------------------------------------------
    % 定义目标函数
    fun=@(x)fitness_fun(train_data, train_label,validate_data,validate_label,x(1),x(2),alpha);       
